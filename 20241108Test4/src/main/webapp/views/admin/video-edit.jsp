    ...
    <div class="container my-4">
        ...
        <form action="${pageContext.request.contextPath}/admin/video/edit" method="post"
            ...
            <div class="form-group mb-3">
                <label>Title:</label>
                <input class="form-control" placeholder="Enter title" name="title" value="${video.title}"
                       required/>
            </div>
            ...
            <div class="form-group mb-3">
                <label>Views:</label>
                <input class="form-control" placeholder="Enter views" name="views" value="${video.views}"
                       required/>
            </div>
            ...
            <div class="form-group mb-3">
                ...
                <select class="form-control" name="categoryId" required>
                    <c:forEach items="${categories}" var="category22162006">
                        <option value="${category22162006.id}"
                                <c:if test="${category22162006.id == video.categoryId}">selected</c:if>>${category22162006.categoryName}</option>
                    </c:forEach>
                </select>
            </div>

            <button type="submit" class="btn btn-primary">Save Changes</button>
            <a href="${pageContext.request.contextPath}/admin/video" class="btn btn-secondary">Back to Video
                List</a>
        </form>
    </div>
